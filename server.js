...
function detectPayerFromPrompt(prompt, org_id) {
  ...
  // Score each candidate against the extracted phrase
  ...
  // 🔥 Dominant match rule
  if (scored.length > 1 && (best.score - scored[1].score) >= 0.25) {
    return best.name;
  }
...
function renderCopilotBriefMessage(result, brief_id, workspace_id){
  ...
  const safeBriefId = safeStr(String(brief_id || "brief")).replace(/[^a-zA-Z0-9_-]/g, "_");
  ...
  return `
    ...
    <div class="ws-msg ws-ai">
      <div class="ws-who">Copilot <span class="badge ok" style="margin-left:8px;">Executive Brief</span></div>
...
function saveAgentWorkspace(org_id, ws){
  const all = readJSON(FILES.agent_workspaces, []);
  const idx = all.findIndex(x => x.workspace_id === ws.workspace_id && x.org_id === org_id);
  const next = { ...ws, org_id, updated_at: nowISO() };
  if (idx >= 0) all[idx] = next;
  else all.push(next);
  writeJSON(FILES.agent_workspaces, all);
  return next;
}

function addAgentMessage(org_id, msg){
  const all = readJSON(FILES.agent_messages, []);
  all.push({ ...msg, org_id, created_at: msg.created_at || nowISO() });
  writeJSON(FILES.agent_messages, all);
}

function getAgentMessages(org_id, billed_id){
  const ws = getAgentWorkspace(org_id, billed_id);
  if (!ws) return [];
  return readJSON(FILES.agent_messages, [])
    .filter(m => m.org_id === org_id && m.workspace_id === ws.workspace_id && m.billed_id === billed_id)
    .sort((a,b)=> new Date(a.created_at||0).getTime() - new Date(b.created_at||0).getTime());
}

function ensureAgentWorkspace(org_id, billedClaim){
  ...
  ws = {
    workspace_id: uuid(),
    org_id,
    billed_id: billedClaim.billed_id,
...
function computePacketCompleteness(ws){
  const req = ((ws?.packet?.required_appeal) || []).filter(i => i.required);
  const total = req.length;
  const complete = req.filter(i => i.status === "present").length;
  const pct = total ? Math.round((complete / total) * 100) : 0;
  return { total, complete, pct };
}

function ensureWorkspacePacket(ws){
  const base = defaultWorkspacePacket();
  const merged = { ...(ws.packet || {}) };
  const mergeArr = (src, cur=[]) => src.map(i => {
    const found = (cur || []).find(x => x.key === i.key) || {};
    return { ...i, ...found };
  });
  merged.required_appeal = mergeArr(base.required_appeal, ws.packet?.required_appeal);
  merged.clinical_support = mergeArr(base.clinical_support, ws.packet?.clinical_support);
  merged.contract_policy = mergeArr(base.contract_policy, ws.packet?.contract_policy);
  merged.completeness = computePacketCompleteness({ packet: merged });
  ws.packet = merged;
  ws.activity = ws.activity || { last_opened_at: null, last_exported_at: null };
  return ws;
}
...
function workspaceNextStep(stage){
  if (stage === "Denied") return "Next: Generate Appeal Draft → Upload Denial Letter → Mark Ready";
  if (stage === "Underpaid") return "Next: Generate Negotiation Draft → Upload EOB/ERA → Mark Ready";
  if (stage === "Patient Follow-Up") return "Next: Adjust Patient Collected or Patient Write-Off";
  if (stage === "In Appeal/Negotiation") return "Next: Update packet → Submit / Follow-up";
  if (stage === "Waiting Payment") return "Next: Upload payment file in Data Management";
...
function storeWorkspaceUpload(org_id, file){
  const dir = path.join(UPLOADS_DIR, org_id, "agent_workspace");
  ensureDir(dir);
  const clean = String(file.filename || "upload.bin").replace(/[^a-zA-Z0-9._-]/g, "_");
  const stored = `${Date.now()}_${uuid()}_${clean}`;
...
function applyAgentInstructionToDraft(draftText, instruction, channel){
  const txt = String(draftText || "");
  const ins = String(instruction || "").trim();
  if (!ins) return txt;
  let next = txt;
  const lower = ins.toLowerCase();
  if (lower.includes("stronger") || lower.includes("firm")) {
    next = next.replace("We request", "We formally request");
  }
...
function getPacketTemplates(){
  return readJSON(FILES.packet_templates, []);
}

function getPacketTemplate(org_id, type){
  const all = getPacketTemplates();
  return all.find(t => t.org_id === org_id && t.type === type) || null;
}

function savePacketTemplate(org_id, type, data){
  const all = getPacketTemplates().filter(t => !(t.org_id === org_id && t.type === type));
  const record = {
    org_id,
    type,
    template_id: uuid(),
  ...
  writeJSON(FILES.packet_templates, all.concat([record]));
  return record;
}

function removePacketTemplate(org_id, type){
  const all = getPacketTemplates();
  const keep = [];
  for (const t of all){
    if (t.org_id === org_id && t.type === type){
      try { if (fs.existsSync(t.stored_path)) fs.rmSync(t.stored_path, { force:true }); } catch {}
    } else {
      keep.push(t);
    }
  }
  writeJSON(FILES.packet_templates, keep);
}
...
function applyTemplate(org_id, type, html){
  const packetType = type === "negotiation" ? "negotiation" : "appeal";
  const tpl = getPacketTemplate(org_id, packetType);
  if (!tpl || !tpl.enabled || !tpl.stored_path || !fs.existsSync(tpl.stored_path)) return html;

  const ext = path.extname(String(tpl.original_filename || "")).toLowerCase();
  const wrapper = fs.readFileSync(tpl.stored_path, "utf8");

  if (ext === ".html" || ext === ".htm") {
    const idx = wrapper.toLowerCase().indexOf("</body>");
    if (idx >= 0) return wrapper.slice(0, idx) + html + wrapper.slice(idx);
    return wrapper + html;
  }
...
if (method === "POST" && pathname === "/ai-copilot/followup") {
  ...
  if (!workspace_id || !prompt) return redirect(res, "/ai-copilot");
...
rowsAdded = toUse;
  ...
  // -------- ACCOUNT --------
  // ===============================
  // Legacy Payer Contracts Redirect Cleanup
  // ===============================

  // Any legacy GET to /payer-contracts
  if (method === "GET" && pathname === "/payer-contracts") {
    return redirect(res, "/data-management?tab=reimbursement");
  }

  // Any legacy POST under /payer-contracts/*
  if (method === "POST" && pathname.startsWith("/payer-contracts")) {
    return redirect(res, "/data-management?tab=reimbursement");
  }
...
function renderTemplateEditor(org, user){
  ...
  const uploadCard = (type, label, tpl) => `
    <div class="tpl-card" style="margin-top:14px;">
      <div class="tpl-row">
        <div>
          <div class="tpl-sub">${label} Template</div>
          <p class="tpl-muted">Current template: ${tpl ? safeStr(tpl.original_filename) : "None uploaded"}</p>
        </div>
        <a class="btn secondary small" href="/packet-template/preview?type=${type}" target="_blank">Preview</a>
      </div>

      <form method="POST" action="/packet-template/upload" enctype="multipart/form-data" style="margin-top:10px;display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;">
        <input type="hidden" name="type" value="${type}"/>
        <div style="flex:1;min-width:250px;">
          <label>Upload File</label>
          <input type="file" name="template_file" accept=".html,.htm,.txt,.docx" required />
        </div>
        <label style="display:flex;align-items:center;gap:8px;font-size:12px;">
          <input type="checkbox" name="enabled" ${tpl?.enabled !== false ? "checked" : ""}/> Enabled
        </label>
        <button class="btn secondary" type="submit">Replace</button>
      </form>

      <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">
        <form method="POST" action="/packet-template/toggle" style="margin:0;display:inline-block;">
          <input type="hidden" name="type" value="${type}"/>
          <input type="hidden" name="enabled" value="${tpl?.enabled === false ? "1" : "0"}"/>
          <button class="btn secondary small" type="submit">${tpl?.enabled === false ? "Enable" : "Disable"}</button>
        </form>
        <form method="POST" action="/packet-template/remove" style="margin:0;display:inline-block;" onsubmit="return confirm('Remove this template?');">
          <input type="hidden" name="type" value="${type}"/>
...
`
  ...
  if (method === "POST" && pathname === "/packet-template/upload") {
    const contentType = String(req.headers["content-type"] || "");
    if (!contentType.includes("multipart/form-data")) return redirect(res, "/data-management?tab=revenue");
    const boundaryMatch = /boundary=([^;]+)/.exec(contentType);
    if (!boundaryMatch) return redirect(res, "/data-management?tab=revenue");

    const parsedMp = await parseMultipart(req, boundaryMatch[1]);
    const fields = parsedMp.fields || {};
    const files = parsedMp.files || [];

    const typeInput = String(fields.type || "appeal").toLowerCase();
    const type = typeInput === "negotiation" ? "negotiation" : "appeal";
    const file = files.find(f => f.fieldName === "template_file") || files[0];
    if (!file) return redirect(res, "/data-management?tab=revenue");

    const ext = path.extname(String(file.filename || "")).toLowerCase();
    if (![".html", ".htm", ".txt", ".docx"].includes(ext)) {
      return redirect(res, "/data-management?tab=revenue&err=bad_ext");
    }

    const dir = path.join(UPLOADS_DIR, org.org_id, "packet_templates", type);
    ensureDir(dir);

    removePacketTemplate(org.org_id, type);

    const stored = path.join(dir, Date.now() + "_" + path.basename(String(file.filename || "template")));
    fs.writeFileSync(stored, file.buffer);

    savePacketTemplate(org.org_id, type, {
      original_filename: file.filename,
      stored_path: stored,
      enabled: fields.enabled === "on" || fields.enabled === "1" || fields.enabled === "true"
    });

    return redirect(res, "/data-management?tab=revenue&ok=template_saved");
  }

  if (method === "POST" && pathname === "/packet-template/remove") {
    const body = await parseBody(req);
    const params = new URLSearchParams(body);
    const type = String(params.get("type") || "appeal").toLowerCase() === "negotiation" ? "negotiation" : "appeal";
    removePacketTemplate(org.org_id, type);
    return redirect(res, "/data-management?tab=revenue&ok=template_removed");
  }

  if (method === "POST" && pathname === "/packet-template/toggle") {
    const body = await parseBody(req);
    const params = new URLSearchParams(body);
    const type = String(params.get("type") || "appeal").toLowerCase() === "negotiation" ? "negotiation" : "appeal";
    const enabled = String(params.get("enabled") || "0") === "1";
    const tpl = getPacketTemplate(org.org_id, type);
    if (tpl) {
      savePacketTemplate(org.org_id, type, {
        original_filename: tpl.original_filename,
        stored_path: tpl.stored_path,
        enabled
      });
    }
    return redirect(res, "/data-management?tab=revenue&ok=template_updated");
  }

  if (method === "GET" && pathname === "/packet-template/preview") {
    const type = String(parsed.query.type || "appeal").toLowerCase() === "negotiation" ? "negotiation" : "appeal";
    const sample = getTemplatePreviewSample(org.org_id);
    const claim = {
      org_id: org.org_id,
      ...
      payer: sample.payer,
      dos: sample.dos,
      amount_billed: sample.amount_billed,
      paid_amount: sample.amount_paid,
      expected_insurance: sample.expected_insurance
    };
  ...
  if (method === "GET" && pathname === "/packet/export") {
    const type = String(parsed.query.type || "appeal").toLowerCase() === "negotiation" ? "negotiation" : "appeal";
    const billed_id = String(parsed.query.billed_id || "").trim();
    const claim = getBilledById(org.org_id, billed_id);
    if (!claim) return redirect(res, "/claims?view=all");
    const derived = evaluateClaimDerived(claim, buildClaimContext(org.org_id));
  ...
  if (method === "POST" && pathname === "/account/templates") {
    const body = await parseBody(req);
    const params = new URLSearchParams(body);

    const next = {
      appeal_opening: String(params.get("appeal_opening") || ""),
    ...
    saveOrgSettings(org.org_id, { letter_defaults: { ...next } });
    saveTemplateSettings(org.org_id, { org_id: org.org_id, ...next, updated_at: nowISO(), updated_by: user.user_id });
    savePracticeSettings(org.org_id, { ...next });

    return redirect(res, "/data-management?tab=revenue&ok=saved");
  }
  ...
  if (method === "POST" && pathname === "/data-management/contracts/add") {
    ...
    if (!payer_name || !procedure_code) return redirect(res, "/data-management?tab=reimbursement");
  ...
  if (method === "GET" && pathname === "/data-management/contracts/history") {
    ...
    if (!contractId) return redirect(res, "/data-management?tab=reimbursement");
  ...
  if (method === "POST" && pathname === "/data-management/contracts/rollback") {
    ...
    if (!version) return redirect(res, "/data-management?tab=reimbursement");
...
if (method === "GET" && pathname === "/agent-workspace") {
  ...
  if (method === "POST" && pathname === "/agent-workspace/save") {
    let body = "";
    req.on("data", c => body += c);
    req.on("end", () => {
      const params = new URLSearchParams(body);
      ...
      if (draft_type === "appeal") {
        ...
        if (lmn) {
          lmn.lmn_text = lmn_text;
          lmn.updated_at = ts;
          lmn.status = lmn_text.trim() ? "present" : (lmn.attachment_id ? "present" : "missing");
        }
      }
  ...
  if (method === "POST" && pathname === "/agent-workspace/status") {
    let body = "";
    req.on("data", c => body += c);
    req.on("end", () => {
      const params = new URLSearchParams(body);
  ...
  if (method === "POST" && pathname === "/agent-workspace/chat") {
    let body = "";
    req.on("data", c => body += c);
    req.on("end", () => {
      const params = new URLSearchParams(body);
      const billed_id = String(params.get("billed_id") || "").trim();
      const channel = String(params.get("channel") || "general").trim();
      const tab = String(params.get("tab") || channel || "activity").trim();
      const content = String(params.get("content") || "").trim();
      const billedAll = readJSON(FILES.billed, []);
      const b = billedAll.find(x => x.billed_id === billed_id && x.org_id === org.org_id);
      if (!b) return redirect(res, "/claims?view=all");
  ...
  if (method === "POST" && pathname === "/agent-workspace/upload") {
    const contentType = String(req.headers["content-type"] || "");
    if (!contentType.includes("multipart/form-data")) return redirect(res, "/claims?view=all");
    const boundaryMatch = /boundary=(.+)$/.exec(contentType);
    if (!boundaryMatch) return redirect(res, "/claims?view=all");
    parseMultipart(req, boundaryMatch[1]).then(({ files, fields }) => {
      const billed_id = String(fields.billed_id || "").trim();
      const packet_key = String(fields.packet_key || "").trim();
      const kind = String(fields.kind || "supporting").trim();
      const tab = String(fields.tab || "packet").trim();
      const billedAll = readJSON(FILES.billed, []);
      const b = billedAll.find(x => x.billed_id === billed_id && x.org_id === org.org_id);
      if (!b) return redirect(res, "/claims?view=all");
      const ws = ensureAgentWorkspace(org.org_id, b);
      ...
      const saved = [];
      for (const f of (files || [])) {
        const up = storeWorkspaceUpload(org.org_id, f);
        ws.attachments = ws.attachments || [];
        const attachment_id = uuid();
        ws.attachments.push({ attachment_id, filename: up.filename, stored_path: up.stored_path, uploaded_at: nowISO(), kind });
        if (packet_key) {
          const item = getWorkspacePacketItem(ws, packet_key);
          if (item) {
            item.status = "present";
  ...
  if (method === "GET" && pathname === "/agent-workspace/export") {
    const billed_id = String(parsed.query.billed_id || "").trim();
    const type = String(parsed.query.type || "appeal").toLowerCase() === "negotiation" ? "negotiation" : "appeal";
    return redirect(res, `/packet/export?billed_id=${encodeURIComponent(billed_id)}&type=${encodeURIComponent(type)}`);
  }

  if (method === "GET" && pathname === "/agent-workspace/file") {
    const attachment_id = String(parsed.query.attachment_id || "").trim();
    const all = readJSON(FILES.agent_workspaces, []).filter(w => w.org_id === org.org_id);
    let found = null;
    for (const ws of all) {
      const at = (ws.attachments || []).find(a => a.attachment_id === attachment_id);
      if (at) { found = at; break; }
    }
    if (!found || !found.stored_path || !fs.existsSync(found.stored_path)) return send(res, 404, "Attachment not found", "text/plain");
    const ext = path.extname(String(found.filename || found.stored_path || "")).toLowerCase();
    const mimeMap = { ".csv": "text/csv", ".pdf": "application/pdf", ".xls": "application/vnd.ms-excel", ".xlsx": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", ".doc": "application/msword", ".docx": "application/vnd.openxmlformats-officedocument.wordprocessingml.document", ".txt": "text/plain", ".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg" };
    res.writeHead(200, { "Content-Type": mimeMap[ext] || "application/octet-stream", "Content-Disposition": `inline; filename="${path.basename(found.filename || "attachment")}"` });
    fs.createReadStream(found.stored_path).pipe(res);
    return;
  }
