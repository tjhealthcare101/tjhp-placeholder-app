 ...
 /**
          ...
          ●​ Dependency: bcryptjs​
             */​
             ...
             const APP_BASE_URL = process.env.APP_BASE_URL || ""; // e.g.
    ...
    *{box-sizing:border-box}​
    ...
    var(--border);border-radius:8px;width:200px;text-align:center;background:#fff;box-shadow:var(-s
    hadow);}​
    ...
    */​
    ...
    initial-scale=1"/> <title>${safeStr(title)}</title> <style>${css}</style> </head> <body> <div
    class="wrap"> <div class="topbar"> <div class="brand"> <h1>TJ Healthcare Pro</h1> <div
    class="sub">AI-Assisted Claim Review & Analytics</div> </div> <div
    class="nav">${navHtml}</div> </div> <div class="card"> ${content} <div class="footer"> No
    EMR access · No payer portal access · No automated submissions · Human review required
    before use. </div> </div> </div> <script> /* ===== PASSWORD VISIBILITY TOGGLE
    (GLOBAL) ===== */ document.querySelectorAll('input[type="password"]').forEach(input => { if
    (input.parentNode && input.parentNode.classList &&
    input.parentNode.classList.contains("password-wrap")) return; const wrap =
    document.createElement("div"); wrap.className = "password-wrap";
    input.parentNode.insertBefore(wrap, input); wrap.appendChild(input); const toggle =
    document.createElement("span"); toggle.className = "toggle-password"; toggle.textContent =
    "Show"; wrap.appendChild(toggle); toggle.addEventListener("click", () => {
    ...
    </body></html>`; } function navPublic() { return `<a href="/login">Login</a><a
    href="/signup">Create Account</a><a href="/admin/login">Owner</a>`; } function navUser() {
    return `<a href="/dashboard">Dashboard</a><a href="/upload">Case Upload</a><a
    href="/payments">Payment Tracking</a><a href="/analytics">Analytics</a><a
    href="/exports">Exports</a><a href="/logout">Logout</a>`; } function navAdmin() { return `<a
    href="/admin/dashboard">Admin</a><a href="/admin/orgs">Organizations</a><a
    href="/admin/audit">Audit</a><a href="/logout">Logout</a>`; } // ===== Models helpers =====
    function getOrg(org_id) { return readJSON(FILES.orgs, []).find(o => o.org_id === org_id); }
    function getUserByEmail(email) { const e = (email || "").toLowerCase(); return
    readJSON(FILES.users, []).find(u => (u.email || "").toLowerCase() === e); } function
    getUserById(user_id) { return readJSON(FILES.users, []).find(u => u.user_id === user_id); }
    function getPilot(org_id) { return readJSON(FILES.pilots, []).find(p => p.org_id === org_id); }
    function ensurePilot(org_id) { const pilots = readJSON(FILES.pilots, []); let p = pilots.find(x =>
    x.org_id === org_id); if (!p) { const started = nowISO(); const ends = addDaysISO(started,
    PILOT_DAYS);
    ...
    if (analyzing >= cap) return { ok:false, reason:Concurrent processing limit reached
    ...
    saveUsage(usage);​
    return { ok:false, reason:AI job rate limit reached (${perHour}/hour). Try
    again shortly. };​
    }​
    return { ok:true };​
    }​
    ...
    if (total >= limits.max_cases_total) return { ok:false, reason:Pilot case limit reached
    (${limits.max_cases_total}). Continue monthly access to review more. };​
    return { ok:true };​
    }​
    ...
    const remaining = Math.max(0, PILOT_LIMITS.payment_records_included
    (usage.pilot_payment_rows_used || 0));​
    ...
    draft_text: To Whom It May Concern, We are writing to appeal the denial
    associated with this claim. Based on the documentation provided, the
    services rendered were medically necessary and appropriately
    supported. Please reconsider the determination after reviewing the
    attached materials. Sincerely, ${orgName || "[Organization Billing
    Team]"} ,​
    ...
              ○​ Case usage ≥ 80%
              ...
              ○​ No payment uploads across platform​
                 */​
                 ...
                 const usage = allUsage.find(u => u.org_id === org.org_id) ||
                 getUsage(org.org_id);​
                 let casePct = 0;​
                 if (limits.mode === 'pilot') {​
                 casePct = (usage.pilot_cases_used / PILOT_LIMITS.max_cases_total) * 100;​
                 } else {​
                 casePct = (usage.monthly_case_credits_used / limits.case_credits_per_month) *
                 100;​
                 }​
    ...
    <h2>Owner Login</h2> <p class="muted">This area is for the system owner only.</p> <form
    method="POST" action="/admin/login"> <label>Email</label> <input name="email" type="email"
    required /> <label>Password</label> <input name="password" type="password" required />
    <div class="btnRow"> <button class="btn" type="submit">Sign In</button> <a class="btn
    secondary" href="/login">Back</a> </div> </form> `, navPublic()); return send(res, 200, html); }
    if (method === "POST" && pathname === "/admin/login") { const body = await parseBody(req);
    ...
    <h2>Owner Login</h2> <p class="error">Admin mode not configured. Set ADMIN_EMAIL and
    ADMIN_PASSWORD_PLAIN (or ADMIN_PASSWORD_HASH) in Railway.</p> <div
    class="btnRow"><a class="btn secondary" href="/admin/login">Back</a></div> `, navPublic());
    return send(res, 403, html); } if (email !== ADMIN_EMAIL || !bcrypt.compareSync(pass, aHash))
    { const html = page("Owner Login", ` <h2>Owner Login</h2> <p class="error">Invalid owner
    credentials.</p> <div class="btnRow"><a class="btn secondary" href="/admin/login">Try
    again</a></div> `, navPublic()); return send(res, 401, html); } const exp = Date.now() +
    SESSION_TTL_DAYS * 86400 * 1000; const token = makeSession({ role:"admin", exp });
    setCookie(res, "tjhp_session", token, SESSION_TTL_DAYS * 86400); return redirect(res,
    "/admin/dashboard"); } // ---------- PUBLIC: Signup/Login/Reset --------- if (method === "GET" &&
    pathname === "/signup") { const html = page("Create Account", ` <h2>Create Account</h2> <p
    class="muted">Secure, organization-based access to AI-assisted claim review and
    analytics.</p> <form method="POST" action="/signup"> <label>Work Email</label> <input
    name="email" type="email" required /> <label>Password (8+ characters)</label> <input
    name="password" type="password" required /> <label>Confirm Password</label> <input
    name="password2" type="password" required /> <label>Organization Name</label> <input
    ...
    <div class="btnRow"> <button class="btn" type="submit">Create Account</button> <a
    class="btn secondary" href="/login">Sign In</a> </div> </form> `, navPublic()); return send(res,
    200, html); } if (method === "POST" && pathname === "/signup") { const body = await
    parseBody(req); const params = new URLSearchParams(body); const email =
    (params.get("email") || "").trim().toLowerCase(); const p1 = params.get("password") || ""; const
    p2 = params.get("password2") || ""; const orgName = (params.get("org_name") || "").trim();
    const ack = params.get("ack"); if (!email || p1.length < 8 || p1 !== p2 || !orgName || !ack) { const
    html = page("Create Account", ` <h2>Create Account</h2> <p class="error">Please complete
    all fields, confirm password, and accept the acknowledgement.</p> <div class="btnRow"><a
    class="btn secondary" href="/signup">Back</a></div> `, navPublic()); return send(res, 400,
    html); } const users = readJSON(FILES.users, []); if (users.find(u => (u.email ||
    "").toLowerCase() === email)) { const html = page("Create Account", ` <h2>Create
    Account</h2> <p class="error">An account with this email already exists.</p> <div
    class="btnRow"><a class="btn" href="/login">Sign In</a></div> `, navPublic()); return send(res,
    400, html); }
    ...
    <h2>Sign In</h2> <p class="muted">Access your organization’s claim review and analytics
    workspace.</p> <form method="POST" action="/login"> <label>Email</label> <input
    name="email" type="email" required /> <label>Password</label> <input name="password"
    type="password" required /> <div class="btnRow"> <button class="btn" type="submit">Sign
    In</button> <a class="btn secondary" href="/signup">Create Account</a> </div> </form> <div
    class="btnRow"> <a class="btn secondary" href="/forgot-password">Forgot password?</a>
    </div>

    , navPublic()); return send(res, 200, html); } if (method === "POST"
    && pathname === "/login") { const body = await parseBody(req); const
    params = new URLSearchParams(body); const email = (params.get("email")
    || "").trim().toLowerCase(); const pass = params.get("password") ||
    ""; const user = getUserByEmail(email); if (!user ||
    !bcrypt.compareSync(pass, user.password_hash)) { const html =
    page("Login",

    <h2>Sign In</h2> <p class="error">The email or password you entered is incorrect.</p> <div
    class="btnRow"><a class="btn secondary" href="/login">Try again</a></div> `, navPublic());
    return send(res, 401, html); } const org = getOrg(user.org_id); if (!org) return redirect(res,
    "/login"); if (org.account_status === "terminated") return redirect(res, "/terminated"); if
    (org.account_status === "suspended") return redirect(res, "/suspended");
    ensurePilot(user.org_id); const exp = Date.now() + SESSION_TTL_DAYS * 86400 * 1000; const
    token = makeSession({ role:"user", user_id:user.user_id, org_id:user.org_id, exp });
    setCookie(res, "tjhp_session", token, SESSION_TTL_DAYS * 86400); return redirect(res,
    "/lock"); } if (method === "GET" && pathname === "/forgot-password") { const html =
    page("Reset Password", ` <h2>Reset Password</h2> <p class="muted">Enter your email to
    generate a reset link (expires in 20 minutes). For v1, the link is shown on-screen.</p> <form
    method="POST" action="/forgot-password"> <label>Email</label> <input name="email"
    type="email" required /> <div class="btnRow"> <button class="btn" type="submit">Generate
    Reset Link</button> <a class="btn secondary" href="/login">Back</a>

    </div>

    </form> `, navPublic()); return send(res, 200, html); } if (method === "POST" && pathname ===
    "/forgot-password") { const body = await parseBody(req); const params = new
    URLSearchParams(body); const email = (params.get("email") || "").trim().toLowerCase(); const
    token = uuid(); const expiresAt = Date.now() + 20*60*1000; const users =
    readJSON(FILES.users, []); const idx = users.findIndex(u => (u.email||"").toLowerCase() ===
    email); if (idx >= 0) { users[idx].reset_token = token; users[idx].reset_expires_at = expiresAt;
    writeJSON(FILES.users, users); } const resetPath =
    `/reset-password?token=${encodeURIComponent(token)}&email=${encodeURIComponent(ema
    il)}`; const resetLink = APP_BASE_URL ? `${APP_BASE_URL}${resetPath}` : resetPath; const
    html = page("Reset Link", ` <h2>Reset Link Generated</h2> <p class="muted">For v1, the
    reset link is displayed here.</p> <div class="btnRow"> <a class="btn"
    href="${safeStr(resetLink)}">Reset Password Now</a> <a class="btn secondary"
    href="/login">Back to login</a> </div> `, navPublic()); return send(res, 200, html); } if (method
    === "GET" && pathname === "/reset-password") { const token = parsed.query.token || ""; const
    email = (parsed.query.email || "").toLowerCase(); const html = page("Set New Password", `
    <h2>Set a New Password</h2> <form method="POST" action="/reset-password"> <input
    type="hidden" name="email" value="${safeStr(email)}"/>
    ...
    <div class="btnRow"> <button class="btn" type="submit">Update Password</button> <a
    class="btn secondary" href="/login">Cancel</a> </div> </form> `, navPublic()); return send(res,
    200, html); } if (method === "POST" && pathname === "/reset-password") { const body = await
    parseBody(req); const params = new URLSearchParams(body); const email =
    (params.get("email") || "").trim().toLowerCase(); const token = params.get("token") || ""; const p1
    = params.get("password") || ""; const p2 = params.get("password2") || ""; if (p1.length < 8 || p1
    !== p2) { const html = page("Set New Password", ` <h2>Set a New Password</h2> <p
    class="error">Passwords must match and be at least 8 characters.</p> <div
    class="btnRow"><a class="btn secondary" href="/forgot-password">Try again</a></div> `,
    navPublic()); return send(res, 400, html); } const users = readJSON(FILES.users, []); const idx =
    users.findIndex(u => (u.email||"").toLowerCase() === email); if (idx < 0) return redirect(res,
    "/login"); const u = users[idx]; if (!u.reset_token || u.reset_token !== token || !u.reset_expires_at
    || Date.now() > u.reset_expires_at) { const html = page("Reset Error", ` <h2>Reset Link
    Invalid</h2> <p class="error">This reset link is expired or invalid.</p> <div class="btnRow"><a
    class="btn secondary" href="/forgot-password">Generate new link</a></div>

    , navPublic()); return send(res, 400, html); }
    users[idx].password_hash = bcrypt.hashSync(p1, 10); delete
    users[idx].reset_token; delete users[idx].reset_expires_at;
    writeJSON(FILES.users, users); return redirect(res, "/login"); } if
    (method === "GET" && pathname === "/logout") { clearCookie(res,
    "tjhp_session"); return redirect(res, "/login"); } if (method ===
    "GET" && pathname === "/suspended") { return send(res, 200,
    page("Suspended",

    <h2>Account Suspended</h2> <p>Your organization’s access is currently suspended.</p> <p
    class="muted">If you believe this is an error, contact support.</p> <div class="btnRow"><a
    class="btn secondary" href="/logout">Logout</a></div> `, navPublic())); } if (method === "GET"
    && pathname === "/terminated") { return send(res, 200, page("Terminated", ` <h2>Account
    Terminated</h2> <p>This account has been terminated. Access to the workspace is no longer
    available.</p> <p class="muted">If you believe this is an error, contact support.</p> <div
    class="btnRow"><a class="btn secondary" href="/logout">Logout</a></div> `, navPublic())); } //
    Shopify activation (manual now) if (method === "GET" && pathname === "/shopify/activate") {
    const token = parsed.query.token || ""; const email = (parsed.query.email || "").toLowerCase();
    const status = parsed.query.status || "inactive"; if (token !== ADMIN_ACTIVATE_TOKEN) return
    send(res, 401, "Unauthorized", "text/plain"); const user = getUserByEmail(email); if (!user) return
    send(res, 404, "User not found", "text/plain");
    ...
    if (casePct >= 80) {​
    alerts.push(${safeStr(org.org_name)} near case limit
    (${casePct.toFixed(0)}%));​
    }​
    ...
    let alertsHtml = alerts.length ? alerts.map(msg => <div
    class="alert">${msg}</div>).join("")​
    ...
    return <tr class="${att ? 'attention' : ''}"> <td><a
    href="/admin/org?org_id=${encodeURIComponent(org.org_id)}">${safeStr(o
    rg.org_name)}</a></td> <td>${plan}</td>
    <td>${safeStr(org.account_status || 'active')}</td> <td>${last ? new
    Date(last).toLocaleDateString() : "—"}</td> <td>${att}</td> </tr>;​
    }).join("");​
    const html = page("Admin Dashboard", `

    <h2>Admin Dashboard</h2> <section> <div class="kpi-card"><h4>Total
    Organisations</h4><p>${totalOrgs}</p></div> <div class="kpi-card"><h4>Total
    Users</h4><p>${totalUsers}</p></div> <div class="kpi-card"><h4>Active
    Pilots</h4><p>${activePilots}</p></div> <div class="kpi-card"><h4>Active
    Subscriptions</h4><p>${activeSubs}</p></div> </section> <h3>Organisation Status</h3> <div
    class="chart-placeholder">Donut chart will render here: Active ${statusCounts['active']||0},
    Suspended ${statusCounts['suspended']||0}, Terminated ${statusCounts['terminated']||0}</div>
    <h3>Total Case Activity</h3> <div class="chart-placeholder">Case activity charts will be
    displayed when data is available.</div> <h3>Admin Alerts</h3> ${alertsHtml} <h3>Recent
    Organisation Activity</h3> <table>
    <thead><tr><th>Name</th><th>Plan</th><th>Status</th><th>Last
    Activity</th><th>Attention</th></tr></thead> <tbody>${rows}</tbody> </table> `, navAdmin());
    ...
    href="/admin/org?org_id=${encodeURIComponent(org.org_id)}">${safeStr(o
    rg.org_name)}</a></td> <td>${plan}</td>
    <td>${safeStr(org.account_status || 'active')}</td> <td>${att}</td>
    </tr>;​
    }).join("");​
    const html = page("Organizations", `

    <h2>Organizations</h2> <form method="GET" action="/admin/orgs"> <input type="text"
    name="search" placeholder="Search org name" value="${safeStr(parsed.query.search || '')}">
    <select name="status"> <option value="">All statuses</option> <option
    value="active"${statusFilter==="active"?" selected":""}>Active</option> <option
    value="suspended"${statusFilter==="suspended"?" selected":""}>Suspended</option> <option
    value="terminated"${statusFilter==="terminated"?" selected":""}>Terminated</option> </select>
    <select name="plan"> <option value="">All plans</option> <option
    value="Pilot"${planFilter==="Pilot"?" selected":""}>Pilot</option> <option
    value="Subscribed"${planFilter==="Subscribed"?" selected":""}>Subscribed</option> <option
    value="Expired"${planFilter==="Expired"?" selected":""}>Expired</option> </select>
    <label><input type="checkbox" name="attention" value="1"${needAtt?" checked":""}> Needs
    Attention</label> <button class="btn" type="submit">Filter</button> </form> <div
    ...
    </table> </div> `, navAdmin()); return send(res, 200, html); } if (method === "GET" &&
    pathname === "/admin/org") { const org_id = parsed.query.org_id || ""; const org =
    getOrg(org_id); if (!org) return redirect(res, "/admin/orgs"); const users =
    readJSON(FILES.users, []).filter(u => u.org_id === org_id); const pilot = getPilot(org_id) ||
    ensurePilot(org_id); const sub = getSub(org_id); const usage = getUsage(org_id); const a =
    computeAnalytics(org_id);
    ...
    const pathOnly =​
    /reset-password?token=${encodeURIComponent(token)}&email=${encodeURICo
    mponent(usr. email)};​
    const full = APP_BASE_URL ? ${APP_BASE_URL}${pathOnly} : pathOnly;​
    return <li class="muted small">${safeStr(usr.email)} — <a
    href="${safeStr(full)}">Reset Link</a></li>;​
    }).join("");​
    const html = page("Org Detail", `

    <h2>${safeStr(org.org_name)}</h2> <div class="row"> <div class="col"> <h3>Account</h3>
    <table> <tr><th>Status</th><td>${safeStr(org.account_status || "active")}</td></tr>
    <tr><th>Plan</th><td>${plan}</td></tr> <tr><th>Pilot ends</th><td>${new
    Date(pilot.ends_at).toLocaleDateString()}</td></tr> <tr><th>Deletion
    date</th><td>${pilot.retention_delete_at ? new
    Date(pilot.retention_delete_at).toLocaleDateString() : "—"}</td></tr> <tr><th>Pilot cases
    used</th><td>${usage.pilot_cases_used || 0}/${PILOT_LIMITS.max_cases_total}</td></tr>
    <tr><th>Pilot payment rows</th><td>${usage.pilot_payment_rows_used ||
    0}/${PILOT_LIMITS.payment_records_included}</td></tr> <tr><th>Drafts
    generated</th><td>${a.drafts}</td></tr> <tr><th>Avg draft time</th><td>${a.avgDraftSeconds ?
    `${a.avgDraftSeconds}s` : "—"}</td></tr> </table> <h3>Actions</h3>
    ...
    <div class="btnRow"> <button class="btn secondary" name="action"
    value="extend_pilot_7">Extend Pilot +7 days</button> <button class="btn secondary"
    name="action" value="suspend">Suspend</button> <button class="btn danger" name="action"
    value="terminate">Terminate</button> </div> </form> </div> <div class="col"> <h3>Users</h3>
    <ul class="muted">${users.map(x => `<li>${safeStr(x.email)}</li>`).join("") || "<li>—</li>"}</ul>
    <div class="hr"></div> <h3>Force Password Reset (v1 display)</h3> <ul>${resetList || "<li
    class='muted small'>No users</li>"}</ul> </div> </div> `, navAdmin()); return send(res, 200,
    html); } if (method === "POST" && pathname === "/admin/action") { const body = await
    parseBody(req); const params = new URLSearchParams(body); const org_id =
    params.get("org_id") || ""; const action = params.get("action") || ""; const reason =
    (params.get("reason") || "").trim(); if (!org_id || !action || !reason) return redirect(res,
    "/admin/orgs"); if (action === "suspend") { setOrgStatus(org_id, "suspended", reason);
    auditLog({ actor:"admin", action:"suspend", org_id, reason }); } else if (action === "terminate") {
    setOrgStatus(org_id, "terminated", reason); // schedule deletion 14 days from now const pilots =
    readJSON(FILES.pilots, []); const idx = pilots.findIndex(p => p.org_id === org_id); if (idx >= 0) {
    ...
    <tr> <td class="muted small">${safeStr(a.at)}</td> <td>${safeStr(a.action)}</td> <td
    class="muted small">${safeStr(a.org_id || "")}</td> <td class="muted small">${safeStr(a.reason
    || "")}</td> </tr>`).join(""); const html = page("Audit Log", ` <h2>Audit Log</h2> <p
    class="muted">Latest 200 admin actions.</p> <div style="overflow:auto;"> <table>
    <thead><tr><th>Time</th><th>Action</th><th>Org</th><th>Reason</th></tr></thead>
    <tbody>${rows}</tbody> </table> </div> `, navAdmin()); return send(res, 200, html); } return
    ...
    patterns that are easy to miss when data lives in different places.</p> <p class="muted
    center">You’ll be guided to the next step automatically.</p> <div class="center"><span
    class="badge warn">Initializing</span></div>
    <script>setTimeout(()=>{window.location.href="/upload";}, ${LOCK_SCREEN_MS});</script> `,
    navUser()); return send(res, 200, html); } // dashboard with empty-state previews and tooltips if
    (method === "GET" && (pathname === "/" || pathname === "/dashboard")) { const limits =
    getLimitProfile(org.org_id); const usage = getUsage(org.org_id); const pilot =
    getPilot(org.org_id) || ensurePilot(org.org_id); const paymentAllowance =
    paymentRowsAllowance(org.org_id); const planBadge = (limits.mode==="monthly") ? `<span
    class="badge ok">Monthly Active</span>` : `<span class="badge warn">Pilot Active</span>`; //
    counts for empty-state charts const caseCount = countOrgCases(org.org_id);

    const paymentCount = readJSON(FILES.payments, []).filter(p => p.org_id ===​
    org.org_id).length;​
    const html = page("Dashboard", `
    <h2>Dashboard</h2> <p class="muted">Organization: ${safeStr(org.org_name)} · Pilot ends:
    ${new Date(pilot.ends_at).toLocaleDateString()}</p> ${planBadge} <div class="hr"></div>
    <h3>Activity</h3> <div class="chart-placeholder">${caseCount === 0 ? "No cases uploaded
    yet." : "Case activity chart will appear here."}</div> <div
    class="chart-placeholder">${paymentCount === 0 ? "No payments uploaded yet." : "Payment
    activity chart will appear here."}</div> ${caseCount === 0 && paymentCount === 0 ? `
    <section><h3>Recommended Next Step</h3><p>Get started by uploading your first case or
    payment data to unlock analytics.</p></section> ` : ""} <div class="hr"></div> <h3>Usage</h3>
    ${limits.mode==="pilot" ? ` <ul class="muted"> <li>Cases remaining:
    ${PILOT_LIMITS.max_cases_total - caseCount} / ${PILOT_LIMITS.max_cases_total} <span
    class="tooltip">ⓘ<span class="tooltiptext">Maximum number of cases you can upload during
    your current plan.</span></span></li> <li>AI jobs/hour:
    ${PILOT_LIMITS.max_ai_jobs_per_hour} <span class="tooltip">ⓘ<span
    class="tooltiptext">Number of AI processing jobs you can run per hour.</span></span></li>
    <li>Concurrent processing: ${PILOT_LIMITS.max_concurrent_analyzing} <span
    class="tooltip">ⓘ<span class="tooltiptext">Maximum number of cases or payments processed
    at the same time.</span></span></li> <li>Payment rows remaining:
    ${paymentAllowance.remaining} of ${PILOT_LIMITS.payment_records_included} <span
    class="tooltip">ⓘ<span class="tooltiptext">Payment table displays up to 500 rows; additional
    rows still count toward your usage.</span></span></li> </ul> `:` <ul class="muted"> <li>Case
    credits used: ${usage.monthly_case_credits_used} / ${limits.case_credits_per_month} <span
    class="tooltip">ⓘ<span class="tooltiptext">Number of cases processed out of your monthly
    allotment.</span></span></li> <li>Overage cases: ${usage.monthly_case_overage_count} (est.
    $${usage.monthly_case_overage_count * limits.overage_price_per_case}) <span
    class="tooltip">ⓘ<span class="tooltiptext">Cases beyond your monthly allotment are charged
    an overage fee.</span></span></li>

    <li>Payment rows remaining: ${paymentAllowance.remaining} <span class="tooltip">ⓘ<span
    class="tooltiptext">Payment table displays up to 500 rows; additional​
    rows still count toward your usage.</span></span></li>

    </ul> `} <div class="btnRow"> <a class="btn" href="/upload">Start Case Review</a> <a
    class="btn secondary" href="/payments">Payment Tracking</a> <a class="btn secondary"
    href="/analytics">Analytics</a> </div> `, navUser()); return send(res, 200, html); } // ---------
    CASE UPLOAD --------- if (method === "GET" && pathname === "/upload") { const html =
    page("Case Upload", ` <h2>Case Upload</h2> <p class="muted">Upload any combination of
    up to <strong>3 documents</strong> for this case. Denial/payment notices alone are enough to
    begin.</p> <form method="POST" action="/upload" enctype="multipart/form-data">
    <label>Documents (up to 3)</label> <div id="case-dropzone" class="dropzone">Drop up to 3
    documents here or click to select</div> <input id="case-files" type="file" name="files" multiple
    required accept=".pdf,.doc,.docx,.jpg,.png" style="display:none" /> <label>Optional
    notes</label> <textarea name="notes" placeholder="Any context to help review
    (optional)"></textarea> <div class="btnRow"> <button class="btn" type="submit">Submit for
    Review</button> <a class="btn secondary" href="/dashboard">Back</a> </div> </form> <div
    class="hr"></div> <p class="muted small">Limits: 3 files/case ·
    ${getLimitProfile(org.org_id).mode==="pilot" ? "10MB/file" : "20MB/file"}</p> <script> const
    caseDrop = document.getElementById('case-dropzone'); const caseInput =
    document.getElementById('case-files'); caseDrop.addEventListener('click', () =>
    caseInput.click()); ['dragenter','dragover'].forEach(evt => { caseDrop.addEventListener(evt, e =>
    { e.preventDefault(); e.stopPropagation(); caseDrop.classList.add('dragover'); }); });
    ...
    pathname === "/upload") { // limit: pilot cases const can =
    pilotCanCreateCase(org.org_id); if (!can.ok) { const html =
    page("Limit",

    <h2>Limit Reached</h2> <p class="error">${safeStr(can.reason)}</p> <div class="btnRow"><a
    class="btn secondary" href="/dashboard">Back</a></div> `, navUser()); return send(res, 403,
    html); } const contentType = req.headers["content-type"] || ""; if
    (!contentType.includes("multipart/form-data")) return send(res, 400, "Invalid upload",
    "text/plain"); const boundaryMatch = /boundary=([^;]+)/.exec(contentType); if (!boundaryMatch)
    return send(res, 400, "Missing boundary", "text/plain"); const boundary = boundaryMatch[1];
    ...
    <h2>Upload</h2> <p class="error">Please upload no more than ${maxFiles} files per case.</p>
    <div class="btnRow"><a class="btn secondary" href="/upload">Back</a></div> `, navUser());
    return send(res, 400, html); } const maxBytes = limits.max_file_size_mb * 1024 * 1024; for
    (const f of files) { if (f.buffer.length > maxBytes) { const html = page("Upload", `
    <h2>Upload</h2> <p class="error">File too large. Max size is ${limits.max_file_size_mb}
    MB.</p> <div class="btnRow"><a class="btn secondary" href="/upload">Back</a></div> `,
    navUser()); return send(res, 400, html); } } // monthly credit consumption (case) if (limits.mode
    === "monthly") monthlyConsumeCaseCredit(org.org_id); else pilotConsumeCase(org.org_id); //
    create case record const case_id = uuid(); const caseDir = path.join(UPLOADS_DIR,
    org.org_id, case_id); ensureDir(caseDir); const storedFiles = files.map((f) => { const safeName
    = (f.filename || "file").replace(/[^a-zA-Z0-9._-]/g, "_"); const file_id = uuid(); const stored_path =
    path.join(caseDir, `${file_id}_${safeName}`); fs.writeFileSync(stored_path, f.buffer); return {
    file_id, filename: safeName, mime: f.mime, size_bytes: f.buffer.length, stored_path,
    ...
    ? <span class="badge warn">Queued</span><p class="muted small">Waiting
    for capacity (rate/concurrency limits).</p>​
    : <span class="badge warn">Analyzing</span><p class="muted small">Draft
    typically prepared within minutes.</p>;​
    const html = page("Status", `

    <h2>Review in Progress</h2> <p class="muted">Our AI agent is analyzing your uploaded
    documents and preparing a draft. This is decision support only.</p> ${badge} <div
    class="hr"></div> <div class="muted small"><strong>Case ID:</strong>
    ${safeStr(case_id)}</div> <script>setTimeout(()=>window.location.reload(), 2500);</script> <div
    class="btnRow"><a class="btn secondary" href="/dashboard">Back</a></div> `, navUser());
    ...
    <h2>Draft Ready for Review</h2> <p class="muted">This workspace supports denial review,
    appeal preparation, claim tracking, and payment analytics using only the documents you
    provide.</p> <div class="badge warn">DRAFT — Editable · For Review Only</div> <div
    class="hr"></div> <h3>Denial Summary</h3> <p>${safeStr(c.ai.denial_summary || "—")}</p>
    <h3>Appeal Considerations</h3> <p>${safeStr(c.ai.appeal_considerations || "—")}</p>
    <h3>Draft Appeal Letter</h3> <form method="POST" action="/draft"> <input type="hidden"
    name="case_id" value="${safeStr(case_id)}"/> <textarea
    name="draft_text">${safeStr(c.ai.draft_text || "")}</textarea> <div class="btnRow"> <button
    class="btn" type="submit">Save Edits</button> <a class="btn secondary"
    href="/download-draft?case_id=${encodeURIComponent(case_id)}">Download TXT</a> <a
    class="btn secondary" href="/analytics">Analytics</a> </div> </form> <p class="muted
    small">Time to draft: ${c.ai.time_to_draft_seconds ? `${c.ai.time_to_draft_seconds}s` :
    "—"}</p> `, navUser()); return send(res, 200, html); } if (method === "POST" && pathname ===
    "/draft") { const body = await parseBody(req);
    ...
    <h2>Payment Tracking (Analytics Only)</h2> <p class="muted">Upload bulk payment files in
    CSV or Excel format.</p> <div class="chart-placeholder">${paymentCount === 0 ? "Upload
    payment data to see trends." : "Payment trend chart will appear here."}</div> <section>
    <h3>What This Unlocks</h3> <ul class="muted"> <li>Identify underperforming payers</li>
    <li>Spot delayed payments early</li> <li>Compare payment trends over time</li> </ul>
    </section> <section>

    <h3>Import Notes</h3>

    <ul class="muted"> <li>CSV files are parsed for analytics only <span class="tooltip">ⓘ<span
    class="tooltiptext">CSV files are used to compute insights but not stored
    permanently.</span></span></li> <li>Excel files are stored but not analysed <span
    class="tooltip">ⓘ<span class="tooltiptext">Excel uploads are stored for record keeping but
    analytics operate on CSV.</span></span></li> <li>500‑row display cap <span
    class="tooltip">ⓘ<span class="tooltiptext">The payment table shows up to 500 rows. Additional
    rows still count towards your usage.</span></span></li> </ul> </section> <p class="muted
    ...
    (analytics-only)</label> <div id="pay-dropzone" class="dropzone">Drop a CSV/XLS/XLSX file
    here or click to select</div> <input id="pay-file" type="file" name="payfile"
    accept=".csv,.xls,.xlsx" required style="display:none" /> <div class="btnRow"> <button
    class="btn" type="submit">Upload for Analytics</button> <a class="btn secondary"
    href="/dashboard">Back</a> </div> </form> <div class="hr"></div> <p class="muted
    small">Note: This does not create appeal drafts. It updates payer and payment timeline
    analytics.</p> <script> const payDrop = document.getElementById('pay-dropzone'); const
    payInput = document.getElementById('pay-file'); payDrop.addEventListener('click', () =>
    ...
    ['dragleave','drop'].forEach(evt => { payDrop.addEventListener(evt, e => { e.preventDefault();
    e.stopPropagation(); payDrop.classList.remove('dragover'); }); });
    payDrop.addEventListener('drop', e => { const files = e.dataTransfer.files; if (files.length > 1) {
    alert('Only one file at a time.'); return; } const dt = new DataTransfer(); dt.items.add(files[0]);
    ...
    boundaryMatch[1]; const { files } = await parseMultipart(req,
    boundary); const f = files.find(x => x.fieldName === "payfile") ||
    files[0]; if (!f) return redirect(res, "/payments"); // validate
    extension const nameLower = (f.filename || "").toLowerCase(); const
    isCSV = nameLower.endsWith(".csv"); const isXLS =
    nameLower.endsWith(".xls") || nameLower.endsWith(".xlsx"); if (!isCSV
    && !isXLS) { const html = page("Payment Tracking",

    <h2>Payment Tracking</h2> <p class="error">Only CSV or Excel files are allowed for payment
    tracking.</p> <div class="btnRow"><a class="btn secondary" href="/payments">Back</a></div>
    `, navUser()); return send(res, 400, html); } // file size cap (use same as plan) const limits =
    getLimitProfile(org.org_id); const maxBytes = limits.max_file_size_mb * 1024 * 1024; if
    (f.buffer.length > maxBytes) { const html = page("Payment Tracking", ` <h2>Payment
    Tracking</h2> <p class="error">File too large. Max size is ${limits.max_file_size_mb} MB.</p>

    <div class="btnRow"><a class="btn secondary" href="/payments">Back</a></div>​
    , navUser()); return send(res, 400, html); } // store raw file const
    dir = path.join(UPLOADS_DIR, org.org_id, "payments"); ensureDir(dir);
    const stored = path.join(dir, ${Date.now()}${(f.filename ||​
    ...
    <h2>Payment File Received</h2> <p class="muted">Your file was uploaded successfully.</p>
    <ul class="muted"> <li><strong>File:</strong> ${safeStr(f.filename)}</li> <li><strong>Rows
    processed:</strong> ${rowsAdded} ${isXLS ? "(Excel not parsed — export to CSV for full
    analytics)" : ""}</li> </ul> <div class="btnRow"> <a class="btn" href="/analytics">View
    Analytics</a> <a class="btn secondary" href="/payments">Upload more</a> </div> `,
    navUser()); return send(res, 200, html); } // analytics page with placeholders and bar chart
    preview if (method === "GET" && pathname === "/analytics") { const a =
    computeAnalytics(org.org_id); const usage = getUsage(org.org_id); const limits =
    getLimitProfile(org.org_id); // Build top 4 payers by total amount const payList =
    Object.entries(a.payByPayer).map(([payer, info]) => ({ payer, total: info.total })); payList.sort((x,
    y) => y.total - x.total); const top4 = payList.slice(0, 4); let payerHtml; if (top4.length === 0) {
    payerHtml = `<div class="chart-placeholder">No payment data available</div>`; } else {
    payerHtml = `<table><thead><tr><th>Payer</th><th>Total Paid</th></tr></thead><tbody>`;
    top4.forEach(item => { payerHtml +=
    `<tr><td>${safeStr(item.payer)}</td><td>$${Number(item.total || 0).toFixed(2)}</td></tr>`;
    ...
    <h2>Analytics</h2> <p class="muted">Derived solely from uploaded documents and
    user‑provided context.</p> <div class="row"> <div class="col"> <h3>Case Distribution</h3>
    <div class="chart-placeholder">${a.totalCases === 0 ? "No cases uploaded" : "Donut chart will
    render here."}</div> </div> <div class="col"> <h3>Payments by Payer</h3> ${payerHtml} </div>
    </div> <div class="hr"></div> <div class="row"> <div class="col"> <h3>Pilot Snapshot</h3> <ul
    class="muted"> <li>Cases uploaded: ${a.totalCases}</li> <li>Drafts generated: ${a.drafts}</li>
    <li>Avg time to draft: ${a.avgDraftSeconds ? `${a.avgDraftSeconds}s` : "—"}</li> </ul> </div>
    <div class="col"> <h3>Usage</h3> ${limits.mode==="pilot" ? ` <ul class="muted"> <li> Pilot
    cases used: ${usage.pilot_cases_used}/${PILOT_LIMITS.max_cases_total} <span
    class="tooltip">ⓘ <span class="tooltiptext">Maximum number of cases allowed during your
    pilot.</span> </span> </li> <li> Pilot payment rows used:
    ${usage.pilot_payment_rows_used}/${PILOT_LIMITS.payment_records_included} <span
    class="tooltip">ⓘ <span class="tooltiptext">The analytics display only 500 rows, but all
    uploaded rows count toward usage.</span>

    </span>

    </li> </ul> `:` <ul class="muted"> <li> Monthly case credits used:
    ${usage.monthly_case_credits_used}/${limits.case_credits_per_month} <span
    class="tooltip">ⓘ <span class="tooltiptext">Number of cases processed out of your monthly
    allotment.</span> </span> </li> <li> Overage cases: ${usage.monthly_case_overage_count}
    (est $${usage.monthly_case_overage_count * limits.overage_price_per_case}) <span
    class="tooltip">ⓘ <span class="tooltiptext">Cases beyond your allotment incur an additional
    fee.</span> </span> </li> <li> Monthly payment rows used:
    ${usage.monthly_payment_rows_used} <span class="tooltip">ⓘ <span class="tooltiptext">The
    analytics display up to 500 rows; all rows still count toward usage.</span> </span> </li> </ul> `}
    </div> </div> `, navUser()); return send(res, 200, html); } // exports hub if (method === "GET"
    && pathname === "/exports") { const html = page("Exports", ` <h2>Exports</h2> <p
    class="muted">Download pilot outputs for leadership and operations review.</p> <div
    class="btnRow"> <a class="btn secondary" href="/export/cases.csv">Cases CSV</a> <a
    class="btn secondary" href="/export/payments.csv">Payments CSV</a>
    ...
    </div> `, navUser()); return send(res, 200, html); } if (method === "GET" && pathname ===
    "/export/cases.csv") { const casesExport = readJSON(FILES.cases, []).filter(c => c.org_id ===
    org.org_id); const header =
    ["case_id","status","created_at","time_to_draft_seconds","denial_reason"].join(","); const rows =
    casesExport.map(c => [ c.case_id, c.status, c.created_at, c.ai?.time_to_draft_seconds || "",
    c.ai?.denial_reason_category || "" ].map(x => `"${String(x).replace(/"/g,'""')}"`).join(",")); const csv
    = [header, ...rows].join("\n"); res.writeHead(200, { "Content-Type":"text/csv",
    "Content-Disposition":"attachment; filename=cases.csv" }); return res.end(csv); } if (method ===
    "GET" && pathname === "/export/payments.csv") { const paymentsExport =
    readJSON(FILES.payments, []).filter(p => p.org_id === org.org_id); const header =
    ["payment_id","claim_number","payer","amount_paid","date_paid","source_file","created_at"].joi
    n(","); const rows = paymentsExport.map(p => [ p.payment_id, p.claim_number, p.payer,
    p.amount_paid, p.date_paid, p.source_file, p.created_at ].map(x =>
    `"${String(x||"").replace(/"/g,'""')}"`).join(",")); const csv = [header, ...rows].join("\n");
    res.writeHead(200, { "Content-Type":"text/csv", "Content-Disposition":"attachment;
    filename=payments.csv" }); return res.end(csv); } if (method === "GET" && pathname ===
    "/export/analytics.csv") { const aExport = computeAnalytics(org.org_id); const header =
    ["metric","value"].join(",");
    ...
    <h2>Pilot Summary Report</h2> <p class="muted">Organization:
    ${safeStr(org.org_name)}</p> <div class="hr"></div> <ul class="muted"> <li>Pilot start: ${new
    Date(pilotRep.started_at).toLocaleDateString()}</li> <li>Pilot end: ${new
    Date(pilotRep.ends_at).toLocaleDateString()}</li> </ul> <h3>Snapshot</h3> <ul
    class="muted"> <li>Cases uploaded: ${aReport.totalCases}</li> <li>Drafts generated:
    ${aReport.drafts}</li> <li>Avg time to draft: ${aReport.avgDraftSeconds ?
    `${aReport.avgDraftSeconds}s` : "—"}</li> </ul> <div class="btnRow"> <button class="btn
    secondary" onclick="window.print()">Print / Save as PDF</button> <a class="btn secondary"
    href="/exports">Back</a> </div> <p class="muted small">All insights are derived from uploaded
    documents during the pilot period.</p> `, navUser()); return send(res, 200, html); } // pilot
    complete page if (method === "GET" && pathname === "/pilot-complete") { const pilotEnd =
    getPilot(org.org_id) || ensurePilot(org.org_id);
    ...
    <h2>Pilot Complete</h2> <p>Your 30-day pilot has ended. Existing work remains available
    during the retention period.</p> <div class="hr"></div> <p class="muted"> To limit unnecessary
    data retention, documents and analytics from this pilot will be securely deleted <strong>14 days
    after the pilot end date</strong> unless you continue monthly access. </p> <ul class="muted">
    <li>Pilot end date: ${new Date(p2.ends_at).toLocaleDateString()}</li> <li>Scheduled deletion
    date: ${p2.retention_delete_at ? new Date(p2.retention_delete_at).toLocaleDateString() :
    "—"}</li> </ul> <div class="btnRow"> <a class="btn" href="https://tjhealthpro.com">Continue
    Monthly Access (via Shopify)</a> <a class="btn secondary" href="/exports">Download
    Exports</a> <a class="btn secondary" href="/logout">Logout</a> </div> `, navUser()); return
    send(res, 200, html); } // fallback return redirect(res, "/dashboard"); }); server.listen(PORT,
    HOST, () => { console.log(`TJHP server listening on ${HOST}:${PORT}`); });
    ...
    */
    ...
    const expected = expectedAmountForClaim({ payer: p.payer, codes: [p.code || p.claimNumber]
    }, contracts);​
    ...
    alerts.push(Payer ${payer} is taking an average of
    ${info.avgDays.toFixed(1)} days to pay.\n);​
    }​
    });​
    if (paymentMetrics.underpaymentValue > thresholds.underpaymentThreshold) {​
    alerts.push(Detected $${paymentMetrics.underpaymentValue.toFixed(2)} in
    underpayments.);​
    }​
    ...
    alerts.push(Average denial risk is high (${riskAvg.toFixed(2)}).
    Investigate category: ${category}.);​
    }​
    });​
    return alerts;​
    }
    ...
    Object.keys(pMetrics.byPayer).length,​
    ...
    const payerCases = cases.filter(c => (c.ai?.denial_category || 'Unknown') !== 'Unknown' &&
    c.ai?.structured_data?.payer === payer);​
    ...
    topDenialCategory: Object.keys(denialMetrics.byCategory).sort((a,b) =>
    denialMetrics.byCategory[b] - denialMetrics.byCategory[a])[0] || 'N/A',​
    ...
    const rows = cards.map(c => Object.values(c).map(v =>
    "${String(v).replace(/"/g,'""')}").join(','));​
    const csv = [header, ...rows].join('\n');​
    res.writeHead(200, { 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment;
    filename=payer_report.csv' });​
    return res.end(csv);​
    }
    ...
    const auditEntries = readJSON(FILES.audit, []).filter(a => a.org_id ===
    orgId).slice(-200).reverse();​
    const rows = auditEntries.map(a => <tr> <td class="muted
    small">${safeStr(a.at)}</td> <td>${safeStr(a.action)}</td> <td
    class="muted small">${safeStr(a.actor)}</td> <td class="muted
    small">${safeStr(a.reason || '')}</td> </tr> ).join('');​
    const html = page('Org Audit', <h2>Audit for Organisation</h2> <table>
    <thead><tr><th>Time</th><th>Action</th><th>Actor</th><th>Reason</th></
    tr></thead> <tbody>${rows}</tbody> </table> , navAdmin());​
    return send(res, 200, html);​
    }
